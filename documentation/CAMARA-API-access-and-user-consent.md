...
- `3-legged access token`: An OAuth2 3-legged access token is a type of token used in the OAuth 2.0 authorisation framework that allows a third-party application to access a user's resources on a resource server on the user's behalf. The term "3-legged" refers to the three parties involved in this flow: the user (the resource owner), the third-party application (the client), and the authorization server/resource server (operator, i.e. service provider).
- `2-legged access token`: A 2-legged access token, also known as a Client Credentials grant, is another type of token used in the OAuth 2.0 framework. Unlike the 3-legged OAuth flow, which involves user interaction, the 2-legged flow is primarily used for server-to-server communication where no user is involved in the authorization process. In the 2-legged flow, only two parties are involved: the third-party application (the client) and the authorization server/resource server (operator, i.e. service provider).
